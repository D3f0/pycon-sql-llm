...
def generate_messages(question, dialect="SQL", top_k=5, table_info=""):
    ...
    messages = [
        {
            "role": msg.type if msg.type != "human" else "user",  # human -> user
            "content": msg.content, 
        }
...
messages = generate_messages(question=question, dialect=db.dialect, table_info=db.get_table_info())
```
---
...
Instead of a provider specific client, we will use 🚅 [*litellm*](https://docs.litellm.ai/docs/providers).
...
### Extracting the SQL with output_format

We could traverse the `response.messages`, but there's a more robust way called 
`output_format` that accepts JSONSchema or a Pydantic `BaseModel`.
...
class SQLOutput(BaseModel):
    ...
    reasoning = Optional[str] = Field(description="Reasoning for the query") 
